...
def analyze_receipt_with_gpt(receipt_name: str):
    image_path = "receipt_image/" + receipt_name
    base64_image = encode_image(image_path)
    lunch = [
    "$5 Meal Deal",
    "Smokey BLT QPC",
    "Double Smokey BLT QPC",
    "Big Mac",
    "Quarter Pounder Burger",
    "McCrispy/McCrispy Deluxe",
    "Spicy McCrispy/Spicy McCrispy Deluxe",
    "Fish Sandwich",
    "McChicken",
    "Chicken Nuggets",
    ...
    prompt = f"On the first line of your response provide the survey code ONLY no whitespace anywhere. On the next four lines please modify the following lists, if an item does not appear in the order, remove it from the list, I want a python syntax list returned on each line, here are the lists:\n{lunch}\n{breakfast_items}\n{mccafe_items}\n{dessert_items}"
    chat_completion = openai_client.chat.completions.create(
        messages=[
            {
                "role": "user",
    ...
    listed_response = response.splitlines()
    survey_code = listed_response[0].strip()
    lunch_list = ast.literal_eval(listed_response[1])
    breakfast_list = ast.literal_eval(listed_response[2])
    mccafe_list = ast.literal_eval(listed_response[3])
    dessert_list = ast.literal_eval(listed_response[4])
    return survey_code, lunch_list, breakfast_list, mccafe_list, dessert_list
...
employee_name = "Felicia"

# take_survey(survey_code, employee_name)
print(analyze_receipt_with_gpt("receipt.jpg"))